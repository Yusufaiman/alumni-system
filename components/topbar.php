...
// Robust dashboard link logic for deep subdirectories
...
// Check if we are deeper in the structure (e.g. student/jobs/...)
// Counting slashes after the base folder might be safer, but strpos is okay for this structure.
$isDeepSubdir = (
    strpos($_SERVER['PHP_SELF'], '/student/jobs/') !== false ||
    strpos($_SERVER['PHP_SELF'], '/student/events/') !== false ||
    strpos($_SERVER['PHP_SELF'], '/student/mentorship/') !== false ||
    strpos($_SERVER['PHP_SELF'], '/student/feedback/') !== false ||
    strpos($_SERVER['PHP_SELF'], '/alumni/jobs/') !== false ||
    strpos($_SERVER['PHP_SELF'], '/alumni/events/') !== false ||
    strpos($_SERVER['PHP_SELF'], '/alumni/mentorship/') !== false ||
    strpos($_SERVER['PHP_SELF'], '/alumni/career/') !== false ||
    strpos($_SERVER['PHP_SELF'], '/alumni/feedback/') !== false ||
    strpos($_SERVER['PHP_SELF'], '/career_officer/jobs/') !== false ||
    strpos($_SERVER['PHP_SELF'], '/career_officer/events/') !== false ||
    strpos($_SERVER['PHP_SELF'], '/career_officer/announcements/') !== false
);
...
if (!$isInsideSubdir) {
    ...
    // If deep subdir, go up one more level
    $prefix = $isDeepSubdir ? '../' : '';

    $dashboardLink = $prefix . "dashboard.php";
    $searchAction = ($isAlumni || $isCareerOfficer ? '../student/' : '') . "alumni_directory.php";
    if ($isDeepSubdir) {
        $searchAction = "../" . $searchAction;
    }

    $logoutAction = $prefix . "../auth/logout.php";
}
?>
...
<header class="top-bar">
    ...
    <nav class="user-nav">
        <!-- Notification Button -->
        <a href="<?php echo (!$isInsideSubdir) ? $folder : $prefix; ?>notifications.php" class="nav-action-btn"
            title="Notifications">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="profile-dropdown-container">
            ...
            <div class="dropdown-menu" id="profileDropdown">
                <a href="<?php echo $prefix; ?>profile.php" class="dropdown-item">My Profile</a>
                <a href="<?php echo $prefix; ?>settings.php" class="dropdown-item">Settings</a>
